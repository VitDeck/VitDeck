...
namespace VitDeck.Validator
{
    ...
    public class SampleRule : BaseRule
    {
        ...
        protected override void Logic(ValidationTarget target)
        {
            ...
            AddIssue(new Issue(null, IssueLevel.Info, "これはサンプルルールの解決策付き検証結果（情報）です。", "解決策テキスト",
                "https://github.com/vitdeck/VitDeck/wiki"));
            var items = "";

            for (int i = 0; i < 10; i++)
                items += "- item" + Environment.NewLine;
            AddIssue(new Issue(null, IssueLevel.Warning, "これはサンプルルールの改行の多い検証結果(警告)です。" + Environment.NewLine + items,
                "解決策テキスト", "https://github.com/vitdeck/VitDeck/wiki"));
            AddIssue(new Issue(null, IssueLevel.Error,
                "これはサンプルルールの検証結果(エラー)です。長い文------------------------------------------------------------章", "解決策テキスト",
                "https://github.com/vitdeck/VitDeck/wiki"));
