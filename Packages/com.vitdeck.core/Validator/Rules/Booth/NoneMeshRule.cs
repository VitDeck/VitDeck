        ...
        protected override void Logic(ValidationTarget target)
        {
            ...
            foreach (var gameObject in gameObjects)
            {
                foreach (var component in gameObject.GetComponents<Component>())
                {
                    ...
                    {
                        var issue = new Issue(skinnedMeshRenderer, IssueLevel.Error,
                            skinnedMeshRenderer + "にメッシュが設定されていません。");
                        AddIssue(issue);
                        continue;
                    }
                    ...
                    if (particleSystem != null)
                    {
                        ...
                        {
                            var issue = new Issue(particleSystem, IssueLevel.Error,
                                particleSystem + "/Shapeがメッシュに依存する設定であるにも関わらず、メッシュが設定されていません。");
                            AddIssue(issue);
                        }
