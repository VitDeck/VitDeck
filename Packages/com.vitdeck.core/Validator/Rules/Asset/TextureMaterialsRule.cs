...
namespace VitDeck.Validator
{
    ...
    {
        private static readonly Regex AutoGeneratedReflectionProbeTextureNamePattern =
            new Regex(@"ReflectionProbe-[0-9]+\.exr");

        private readonly string platform;
        private readonly IEnumerable<TextureImporterFormat> importerFormats;

        public TextureImporterFormatRule(string name, string platform,
            IEnumerable<TextureImporterFormat> importerFormats) : base(name)
        {
        ...
        protected override void Logic(ValidationTarget target)
        {
            ...
            var autoGeneratedFolderPaths = target.GetScenes()
                .Select(scene =>
                    Path.Combine(Path.GetDirectoryName(scene.path), Path.GetFileNameWithoutExtension(scene.path)));
