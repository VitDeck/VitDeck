...
namespace VitDeck.Exporter
{
    public class ExportSetting : ScriptableObject
    {
        ...
        [Tooltip(
            "File name for export. If `{SHA-1}` is contained, it will replace SHA-1 hash value. If `{ID}` is contained, it will replace base folder name. If `{DATETIME}` is contained, it will replace exporting time.")]
        ...
        [Multiline(lines: 40),
         Tooltip(
             "Extensions of the file to include in the unitypackage. A newline-separated extensions starting with `.`. Case-insensitive. If this field is empty, all files will be exported.")]
